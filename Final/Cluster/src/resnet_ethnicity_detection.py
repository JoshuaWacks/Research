...
dataset = 'FairFace_ethnicity_training'
...
def train_model(model, criterion, optimizer, scheduler,start_epoch, num_epochs=25):
	...
	for epoch in range(start_epoch,num_epochs):
		...
		for phase in ['train', 'val']:
			...
			save_checkpoint(model, optimizer, os.path.expanduser(full_path),epoch)
...
if load_presaved_model :
  model_ft, optimizer, starting_epoch = load_checkpoint(model_ft, optimizer_ft, os.path.expanduser(full_path))
...
def write_results_drive(time_elapsed,training_loss,training_acc,validation_loss,validation_acc):
	...
	path = r'~/Results/FairFace_ethnicity_training/results.csv'
	...
	path = r'~/Results/FairFace_ethnicity_training/time_results.csv'
	with open(os.path.expanduser(path),'w') as csvfile:
